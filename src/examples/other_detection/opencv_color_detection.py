...
class Color(ShowPlayer):
    ...
    def _detect(self):
        hsv = cv.cvtColor(self.frame_in, cv.COLOR_BGR2HSV)
        ...
        res_red = cv.bitwise_and(self.frame_out, self.frame_in, mask=red_mask)

        green_mask = cv.dilate(green_mask, transform)
        res_green = cv.bitwise_and(self.frame_out, self.frame_in, mask=green_mask)

        blue_mask = cv.dilate(blue_mask, transform)
        res_blue = cv.bitwise_and(self.frame_out, self.frame_in, mask=blue_mask)
        ...
        for pic, contour in enumerate(contours):
            ...
            if area > 300:
                ...
                cv.putText(self.frame_out, "Red", (x, y), cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255))
        ...
        for pic, contour in enumerate(contours):
            ...
            if area > 300:
                x, y, w, h = cv.boundingRect(contour)
                cv.rectangle(self.frame_out, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv.putText(self.frame_out, "Green", (x, y), cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0))
        ...
        for pic, contour in enumerate(contours):
            ...
            if area > 300:
                x, y, w, h = cv.boundingRect(contour)
                cv.rectangle(self.frame_out, (x, y), (x + w, y + h), (255, 0, 0), 2)
                cv.putText(self.frame_out, "Blue", (x, y), cv.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0))
